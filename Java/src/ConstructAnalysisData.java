...
public class ConstructAnalysisData {
	...
	private AnalyzedPacket pktAnalysis;
	...
	public void initialize(int choice) {
		...
		if (choice == 7) {
			...
			this.pktAnalysis = new AnalyzedPacket();
			this.pkts_out = new ArrayList<>();
		} else if (choice == 11) {
	 ...
	 * export for each flow <srcIP, dstIP>
	 * analysis.constructIOAValues();
	 * analysis.printIOAValues();
	 */
	public void constructIOAValues(int choice) {
		initialize(choice);
	...
	public void constructIOAClusterPerFlow(int choice) {
		initialize(choice);
		ioaCluster.buildIOAClusterPerFlow(pkts, analyzedDataMap, choice);
		numMalformedPackets = ioaCluster.getNumMalformedPackets();
		setClusterType(4);
	}
	...
	public void constructTimeSeries(int choice) {
		initialize(choice);
		...
		//csvFileWriter.continueWriter(srcMap, "srcIP_vs_time.csv");
		//HashMap<Double, Integer> dstMap = timeSeries.getDstIpTime();
		//HashMap<Double, Integer> ipMap = timeSeries.getIpTime();
	...
	public void constructAPCIAnalysis(int choice) {
		initialize(choice);
	}
	...
	public void constructPacketAnalysis(int choice) {
		initialize(choice);
		pktAnalysis.basics(pkts, pkts_out, choice);
		//String h = "srcIP,dstIP,epoch_time,relative_time,APDU_Type,APDU_Length";
		//csvFileWriter1.apduExport(h, apdus);
	}
	// @XI
